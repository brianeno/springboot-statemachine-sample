...
@Slf4j
@Component
public class EditingCompleteAction implements Action<ArticleStates, ArticleEvents> {
    ...
    public void execute(StateContext<ArticleStates, ArticleEvents> context) {
        ...
        context.getStateMachine().sendEvent(Mono.just(MessageBuilder.withPayload(ArticleEvents.REVIEW_ARTICLE)
            .setHeader(ArticleServiceImpl.ARTICLE_ID_HEADER, header)
            .build())).doOnComplete(() -> System.out.println("Editing Article complete")).subscribe();
    }
}
